...
- **Surfaces:** Warm white cards (`#ffffff` → `#fefefe` or barely-warm), borders stay subtle. Elevation through shadow, not heavy outlines.
...
- **Dark mode:** Support `prefers-color-scheme: dark`. Swap to dark slate backgrounds (`#0f172a` / `#1e293b`), warm text (`#f1f0ee`), muted borders. Indigo accents remain legible.
- **Anti-references:** Avoid generic SaaS blue (`#1a56db` as-is), clinical white + gray dashboards, anything that feels like an HR software product.
